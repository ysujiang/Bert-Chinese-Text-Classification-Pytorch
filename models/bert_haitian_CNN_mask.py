...
class Config(object):
    ...
    def __init__(self, dataset):
        self.model_name = 'bert_CNN_haitian_CNN_mask_615_5'
...
class Model(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.fc_mask = nn.Linear(768*3,config.num_classes)
    ...
    def forward(self, x):
        ...
        #print('polyphones shape ',polyphones.shape)
        ...
        #print('pos embedding ',pos_embedding.shape,segment_embedding.shape)
        encoder_out, text_cls = self.bert(context, attention_mask=mask, output_all_encoded_layers=False)
        #print('bert encoder shape ',encoder_out.shape)
        #print('position',position.shape,len(position))
        
        bert_mask = torch.cat([encoder_out[i,position[i]] for i in range(len(position))])
        pos_mask = torch.cat([pos_embedding[i,position[i]] for i in range(len(position))])
        segment_mask = torch.cat([segment_embedding[i,position[i]] for i in range(len(position))])
        #print('mask shape',bert_mask.shape,pos_mask.shape,segment_mask.shape)
        ''' 
        bert_encoder_time= time.clock()
        #print('bert encoder ',bert_encoder_time-start_time)
        all_mask = torch.cat([bert_mask.unsqueeze(1),pos_mask.unsqueeze(1),segment_mask.unsqueeze(1)],dim=1)
        #print('all mask ',all_mask.shape)
        bert_mask = self.dropout(all_mask)
        bert_mask = bert_mask.view(bert_mask.size(0),-1)
        #print('after mask',bert_mask.shape)
        out = self.fc_mask(bert_mask)
        #print('out shape',out.shape)
        polyphone = polyphones.type_as(out)
        ...
        if test:
            ...
            for i in range(polyphones.size(1)):
                ...
                all_mask = torch.cat([bert_mask[i,:,:].unsqueeze(1),pos_mask[i,:,:].unsqueeze(1),segment_mask[i,:,:].unsqueeze(1)],dim=1)
                all_mask = self.dropout(all_mask)
                all_mask = all_mask.view(all_mask.size(0),-1)
                out = self.fc_mask(all_mask)
                
                print('other time ',time.clock()-other_start)
                polyphone = polyphones[:,i,:].type_as(out)
               
                #print('ployphone shape',polyphone.shape)
                out = polyphone*torch.exp(out)/torch.sum(polyphone*torch.exp(out),dim=1).view(-1,1)
