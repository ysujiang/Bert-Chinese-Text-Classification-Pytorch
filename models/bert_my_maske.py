...
class Config(object):
    ...
    def __init__(self, dataset):
        ...
        self.num_epochs = 30 #30                                             # epoch数
        self.batch_size = 128#64                                           # mini-batch大小
        self.pad_size =51 #64 #32                                              # 每句话处理成的长度(短填长切)  多了一个开始的位置
...
class Model(nn.Module):
    ...
    def __init__(self, config):
       ...
       # self.embedding = nn.Embedding(num_embeddings=config.vocab_size, embedding_dim = config.embedding_size)
        ...
        self.lstm = nn.LSTM(config.hidden_size+1, config.rnn_hidden, config.num_layers,
    ...
    def forward(self, x):
       ...
       # print(len(x))
        ...
        #print(positions[1])
        #print(polyphones[1])
        token = self.config.tokenizer.convert_ids_to_tokens(
context[1].cpu().numpy())
        #print(token)
